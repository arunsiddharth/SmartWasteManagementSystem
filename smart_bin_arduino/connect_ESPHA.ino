boolean connect_ESPHA(){
  ...
  ESP8266.print("AT+CIPSTART=0,\"TCP\",\"espha.000webhostapp.com\",80\r\n");

  if(read_until_ESP(keyword_OK,sizeof(keyword_OK),15000,0)){
    ...
    //LOOK FOR '>'
      if(read_until_ESP(keyword_carrot,sizeof(keyword_carrot),15000,0)){
        Serial.println("READY TO SEND");
        Serial.println("Connected To PREINTERN");
      for(int i=0; i<payload_size; i++)//print the payload(page url) to the ESP
        ESP8266.print(payload[i]);

      //SEND it to SITE
      if(read_until_ESP(keyword_sendok,sizeof(keyword_sendok),15000,0)){
        Serial.println("SENT");
